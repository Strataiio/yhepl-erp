...
export default function Page() {
  ...
  useEffect(() => {
    supabase.from('qc_checkpoints').select('*').order('created_at', { ascending:false }).limit(100).then(({ data }) => { setRows(data||[]); setLoading(false) })
  }, [])
  ...
  return (
    <div style={{ padding:'32px', maxWidth:'1000px', fontFamily:'system-ui,sans-serif' }}>
      <PageHeader title="QC checkpoints" subtitle="Inspection gates — blocking or advisory — per process" />
