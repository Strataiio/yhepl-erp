...
export default function Page() {
  ...
  async function load(){
    ...
    try { setRows(await loadData('machines')) } catch(e){ setError(String(e)) }
    setLoading(false)
  }
  useEffect(()=>{load()},[])

  const cols = ['code|name|machine_type|make|model|cost_per_hour|is_active'.split('|')]

  return (
    <div style={{padding:'32px',maxWidth:'1000px',fontFamily:'system-ui,sans-serif'}}>
      <PageHeader title="Machines & assets" subtitle="Machine registry with cost/hr and station links" />
