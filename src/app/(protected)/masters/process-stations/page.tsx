...
export default function Page() {
  ...
  async function load(){
    ...
    try { setRows(await loadData('process_stations')) } catch(e){ setError(String(e)) }
    setLoading(false)
  }
  useEffect(()=>{load()},[])

  const cols = ['code|name|capacity_hrs_per_shift|shifts_per_day|plant_area|is_active'.split('|')]

  return (
    <div style={{padding:'32px',maxWidth:'1000px',fontFamily:'system-ui,sans-serif'}}>
      <PageHeader title="Process stations" subtitle="Shop floor stations with capacity" />
