...
export default function Page() {
  ...
  async function load(){
    ...
    try { setRows(await loadData('cost_rates')) } catch(e){ setError(String(e)) }
    setLoading(false)
  }
  useEffect(()=>{load()},[])

  const cols = ['name|rate_type|category|rate_per_hour|effective_from|is_active'.split('|')]

  return (
    <div style={{padding:'32px',maxWidth:'1000px',fontFamily:'system-ui,sans-serif'}}>
      <PageHeader title="Cost rates" subtitle="Labour, machine and overhead rates" />
