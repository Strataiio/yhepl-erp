...
export default function Page() {
  ...
  async function load(){
    ...
    try { setRows(await loadData('product_types')) } catch(e){ setError(String(e)) }
    setLoading(false)
  }
  useEffect(()=>{load()},[])

  const cols = ['code|name|description|is_active'.split('|')]

  return (
    <div style={{padding:'32px',maxWidth:'1000px',fontFamily:'system-ui,sans-serif'}}>
      <PageHeader title="Product types" subtitle="Products that transform through buckets" />
