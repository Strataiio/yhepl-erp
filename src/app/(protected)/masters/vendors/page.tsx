...
export default function Page() {
  ...
  async function load(){
    ...
    try { setRows(await loadData('vendors')) } catch(e){ setError(String(e)) }
    setLoading(false)
  }
  useEffect(()=>{load()},[])

  const cols = ['code|name|category|city|phone|is_active'.split('|')]

  return (
    <div style={{padding:'32px',maxWidth:'1000px',fontFamily:'system-ui,sans-serif'}}>
      <PageHeader title="Vendors" subtitle="Plate suppliers, service vendors, NDE agencies" />
