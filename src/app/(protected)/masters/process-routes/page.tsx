...
export default function Page() {
  ...
  useEffect(() => {
    supabase.from('process_routes').select('*').order('created_at', { ascending:false }).limit(100).then(({ data }) => { setRows(data||[]); setLoading(false) })
  }, [])
  ...
  return (
    <div style={{ padding:'32px', maxWidth:'1000px', fontFamily:'system-ui,sans-serif' }}>
      <PageHeader title="Process routes" subtitle="Route templates per product type — Fixed, Parallel, Conditional" />
