...
export default function Page() {
  ...
  async function load(){
    ...
    try { setRows(await loadData('material_grades')) } catch(e){ setError(String(e)) }
    setLoading(false)
  }
  useEffect(()=>{load()},[])

  const cols = ['code|name|standard|ys_min|uts_min|is_active'.split('|')]

  return (
    <div style={{padding:'32px',maxWidth:'1000px',fontFamily:'system-ui,sans-serif'}}>
      <PageHeader title="Material grades" subtitle="IS / ASME / EN grades with composition limits" />
